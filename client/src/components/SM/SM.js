...
function SM() {
  ...
  const handleUpClick = () => {
    handleRunClick('sm', 'start');
  };

  const handleDownClick = () => {
    handleRunClick('sm', 'stop');
  };
  ...
  return (
    <div className="container" style={{ backgroundColor: "#e9ecef" }}>
      <h1 className="mb-4 text-dark">Security Misconfiguration (SM)</h1>
      <div className="container">
        <div className="row">
          <div className="col-sm-3">
          </div>
          <div className="col-sm-6 d-flex justify-content-between">
            <button className="btn btn-primary mt-3 col-sm-5" onClick={handleUpClick}>Start SM Instance</button>
            <button className="btn btn-danger mt-3 col-sm-5" onClick={handleDownClick}>Stop SM Instance</button>
          </div>
